...
**전부 권고안으로 확정했다.** 사용자 지시: "즉시 구현이 가능할 때까지 멈추지 마".
따라서 blocking 미결 3건을 각 절의 권고안대로 채택했다. 되돌리는 비용은 아래
각 항목에 적혀 있다.

| 미결 | 채택한 답 | 되돌리는 비용 |
|---|---|---|
| O1 Inspector 헤더 | **A) 5행으로 늘리고 `date:` 행 독립** | 헤더 높이 계약 4→5, 높이 12 화면에서 diff 1줄 |
| O2 날짜 종류 | **A) author와 committer가 다를 때만 둘 다** | 조건문 하나 + `%aI` 제거 |
| O3 하이라이팅 다음 | **A) 표시 + 요약만** | 없음 (B/C는 순수 추가 확장) |
| O4 분기 시각화 자리 | **A) 마커 어휘를 확장 가능하게, 이번엔 1종** | 없음 |

O1+O2 조합의 결과: 헤더는 **5행이 기본, author/committer가 갈린 커밋에서만 6행**이다.
`commit_inspector_screen_test.go`의 높이 매트릭스(12/20/30)를 6행 기준으로 재확인한다.

**타입 레벨 계약은 이 문서가 아니라 spec 문서가 소유한다** —
`docs/20260908-0001-graph-dates-and-commit-range-spec.md`. 계약을 두 문서에 적으면
`TODOS.md`에 방금 등록한 "같은 항목이 두 곳에 있어 리뷰어가 오판한다"는 결함을
그대로 재생산한다. 이 문서는 **왜·무엇을**, spec 문서는 **정확히 어떤 시그니처로**를
소유한다.
...
1. `repo_parse.go:186` 포맷 문자열 `%x00%H%x1f%P%x1f%ar%x1f%an%x1f%D%x1f%s`에
   **`%cI`를 맨 끝에 붙인다.**

   **정정 (2026-09-08, 실측).** 이 문단의 초안은 `%cs` + "Inspector에서만
   `--date=iso-strict`"를 지시했다. 더 나은 방법이 있다. `%cI`(committer date, strict
   ISO 8601)는 **`--date` 플래그 없이** 원본 오프셋을 그대로 준다. 실측:

   ```
   $ git log -1 --format='cI=%cI%ncs=%cs'
   cI=2026-09-08T23:15:16+09:00
   cs=2026-09-08
   ```

   `%cI`를 쓰면 (a) `graphLogArgs`에 새 인자가 없고, (b) P2의 원본 오프셋 요구를
   데이터 계층에서 만족하고, (c) `%cs`가 버리는 시각을 보존하므로 P1이 Details에
   요구한 `YYYY-MM-DD HH:MM`이 가능하다. 데이터 계층은 최대 정밀도를 들고, 좁히는
   것은 렌더 계층이 한다. Inspector도 같은 이유로 `%aI`/`%cI`를 쓰며 `--date` 인자가
   필요 없다.
