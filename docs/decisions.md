...
## 2026-09-10 — numstat 파서가 두 형식 중 드문 쪽만 알고 있었다 (task 9.16, 9.19)

9.16 의 범위는 "`--numstat` 에 `-M -C` 를 붙인다"였다. 붙이기 전에 파서를
측정했더니 **그보다 앞선 결함**이 있었다.

`git diff-tree --numstat -z` 는 레코드 형식이 두 개다.

```
일반   add \t del \t path \0
rename add \t del \t     \0 old \0 new \0     (-M 이 있을 때만)
```

파서는 세 필드씩 건너뛰며 세 번째를 경로로 읽었다 — **rename 형식**이다.
그런데 `-M` 이 없었으므로 rename 형식은 **한 번도 나오지 않았다.** 일반 커밋에서
레코드는 레코드당 한 필드라, 파서는 빈 문자열에 첫 파일의 카운트를 매핑하고
실제 경로는 하나도 찾지 못했다.

**측정.** 두 파일짜리 평범한 커밋에서 `byPath = {"": ("1","1")}`.
즉 **모든 파일의 additions·deletions 가 0** 이었다.

코드 주석은 이 증상을 git 동작으로 설명해 두고 있었다 — "Git does not emit a
direct path record". git 은 레코드를 냈다. 파서가 다른 자리를 보고 있었다.

**딸린 결과 둘. 각각 별도 결함으로 등록돼 있던 것들이다.**

- binary 파일은 `-\t-\tpath` 레코드다. 파서가 거기 닿지 못했으니 **binary 가
  한 번도 탐지되지 않았고** Inspector 의 binary 마커는 발화할 수 없었다.
  이것이 9.19 가 "어댑터가 Binary 를 설정하지 않는다"로 본 증상의 진짜 원인이다.
  어댑터는 정상이었다.
- `--summary` 패스는 "mode change" 가 있고 카운트가 0 이면 ModeOnly 로 표시한다.
  카운트가 항상 0 이었으므로, 내용이 바뀐 파일이라도 mode change 만 있으면
  ModeOnly 로 덮였다.

파서를 두 형식 모두 읽게 고치고, `-M -C` 를 numstat 과 --summary 에 붙여
name-status(이미 갖고 있었다)와 말이 맞게 했다. 실측: `a.txt +1 -0`,
`img.bin binary=true`, rename 이 두 파일이 아니라 한 파일로 `+1 -0`.

**빈 키는 다시 못 들어온다.** 잘린 rename 레코드의 "새 경로"가 마지막 NUL 뒤
빈 꼬리에 떨어지면 예전과 같은 빈 키가 생긴다. 명시적으로 막고 테스트했다.

## 2026-09-10 — 산문의 하드컷은 D-007 이 놓친 자리였다

D-007 은 축약 마커를 하나로 통일하면서 `shorten` 과 리터럴 `"..."` 를 정리했지만
`fitVisibleWidth` 는 "마커 없는 하드컷"으로 분류하고 그대로 뒀다. **언제 하드컷이
허용되는지는 정하지 않았다.**

**렌더된 프레임을 눈으로 보고 찾았다.** 80컬럼에서 푸터가 `ctrl + u/d: s`,
Tags 패널이 `Press F to sy` 로 잘려 있었다. 테스트는 전부 통과하고 있었다 —
어떤 테스트도 좁은 폭의 산문을 보고 있지 않았기 때문이다.

**규칙:** `fitVisibleWidth` 는 **이미 폭에 맞춰 만든 것**의 프리미티브다 —
그래프 헤더, `… +N hidden` 표시처럼 구조적으로 폭이 정해진 것. 마커가 붙으면
소음이다. **박스보다 커질 수 있는 문장**은 `truncateText` 를 쓴다. 푸터, Tags 안내,
graph state hint, cherry-pick 요약이 여기 해당했다.
...
## 2026-09-10 — 탭은 폭이 0으로 세어지고 있었다

`lipgloss.Width("\t")` 는 **0** 이다. 터미널은 탭을 최대 8칸으로 그린다.
따라서 앱의 모든 폭 계산이 **들여쓴 코드 줄을 실제보다 짧게** 쟀고, fit 검사가
그대로 통과시켰으며, 터미널이 줄바꿈했다. 넘친 나머지는 그 행의 두-pane 구분선을
깨뜨렸다. diff 에 나오는 Go 코드는 거의 전부 들여쓰기가 있다.

**렌더된 프레임을 읽어 찾았다.** `TestInspectorScreenDoesNotWrapLongDiffCode` 는
이걸 놓쳤다 — 그 테스트의 긴 줄에는 탭이 없었다.

**고침:** diff 텍스트가 렌더러에 들어가는 단일 통로(`formatInspectorDiffLine`)에서
탭을 공백으로 펼친다. 탭 스톱 **4**칸 — pane 은 터미널의 절반이고 그 안에 보이는
것 대부분이 들여쓴 코드다. 펼치고 나면 앱이 재는 문자열이 터미널이 그리는
문자열과 같아진다.

이걸로 오늘 밤 시각 검증이 찾은 결함은 셋이다 — 마커 없는 산문 하드컷,
폼 팝업의 빈 공간, 그리고 이것. 셋 다 테스트는 전부 통과하는 상태였다.

### 시각 검증 3회차 — 패널 제목과 "+N hidden"

60컬럼에서 `╭ Graph Detai ╮` 와 `… +2 hidd` 가 마커 없이 잘려 있었다.
패널 제목은 **이름**이다 — 잘린 이름은 짧아진 이름이 아니라 다른 패널로 읽힌다.
브랜치 이름을 마킹하는 것과 같은 논리다. `… +N hidden` 은 문장이고, 잘리면
표시로서의 의미가 사라진다. 둘 다 `truncateText` 로 옮겼다.
