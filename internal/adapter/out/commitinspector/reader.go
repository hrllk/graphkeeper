...
func (r *gitCommitInspectorReader) InspectCommit(ctx context.Context, req commitinspector.CommitRequest) commitinspector.InspectorResult[commitinspector.CommitSnapshot] {
	...
	for index, file := range inspection.Files {
		...
		files = append(files, commitinspector.ChangedFile{StableID: stableID, Status: mapInspectorStatus(file), OldPath: file.OldPath, Path: file.Path, Additions: file.Additions, Deletions: file.Deletions, Binary: file.Binary})
	}
...
func (r *gitCommitInspectorReader) LoadDiff(ctx context.Context, req commitinspector.DiffRequest) commitinspector.InspectorResult[commitinspector.DiffWindow] {
	...
	window := commitinspector.DiffWindow{FileID: req.FileID, HasMore: diff.HasMore, PartialReason: partialReason, Hunks: diffWindowHunks(lines, boundedRows), NextStartLine: diff.NextStartLine}
